...
keg_cli_3_0_0_update(){
  ...
  keg cli env set --key KEG_BASE_IMAGE --value \"ghcr.io/keg-hub/keg-base:{{ cli.settings.docker.defaultTag }}\" --confirm false
  keg cli env set --key KEG_IMAGE_FROM --value \"ghcr.io/keg-hub/keg-base:{{ cli.settings.docker.defaultTag }}\" --confirm false
  ...
  keg config set --key cli.git.orgUrl --value \"https://github.com/keg-hub\" --confirm false
  ...
  keg config set --key docker.namespace --value keg-hub --confirm false
