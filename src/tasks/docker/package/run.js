...
const getKegProxyPort = (ports, proxyPort, kegProxyPort, { labels, ports:imgPorts }) => {
  ...
  return proxyPort ||
    kegProxyPort ||
    labels[kegLabelKeys.KEG_ENV_PORT] ||
    get(imgPorts, '0') ||
...
const checkProxyUrl = (optsWLabels, imgLabels, args) => {
  const subdomain = imgLabels && imgLabels[kegLabelKeys.KEG_PROXY_DOMAIN]
  ...
  const { builtOpts, fullProxyUrl } = subdomain
    ? getOptsWithProxyRule(subdomain, optsWLabels)
    ...
    options: pickKeys(
      mergeTaskOptions(`docker package`, 'run', 'run', {
        package: {
          ...
          example: `keg docker package --package keg-hub/keg-core/keg-core:bug-fixes`,
          required: true,
          ask: {
