...
const stopService = async (args, argsExt) => {
  ...
  await runInternalTask('docker.tasks.compose.tasks.stop', serviceArgs)
  const service = serviceArgs.params.tap || serviceArgs.params.context
  ...
  !checkEnvConstantValue(service, 'KEG_AUTO_SYNC', false) &&
